...
"""
from DeepCAD_RT.deepcad.test_collection import testing_class
# from DeepCAD_RT.deepcad.movie_display import display
# from DeepCAD_RT.deepcad.utils import get_first_filename,download_demo

def deep_denoise(reg_image,model):
    # %% Select file(s) to be processed (download if not present)
    # download_demo_file = False
    # if download_demo_file:
    #     file_name='mouse_spine' # select the demo file you want to test (e.g. 'ATP_3D', 'fish_localbrain', 'NP_3D', ...)
    #     datasets_path, denoise_model = download_demo(download_filename=file_name)
    # else:
    #datasets_path = 'datasets/raw'  # folder containing tif files for testing
    ...
    test_dict = {
        ...
        # xxxx'datasets_path': datasets_path,
        'pth_dir': './DeepCAD_RT/pth',                 # pth file root path
    ...
    enhance_image = tc.run(reg_image)
