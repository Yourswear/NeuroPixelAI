...
class trainset(Dataset):
    ...
    def __init__(self, name_list, coordinate_list, reg_image_all, stack_index):
        self.name_list = name_list
        self.coordinate_list = coordinate_list
        self.reg_image_all = reg_image_all
    ...
    def __getitem__(self, index):
        ...
        reg_image = self.reg_image_all[stack_index]
        ...
        input = reg_image[init_s:end_s:2, init_h:end_h, init_w:end_w]
        target = reg_image[init_s + 1:end_s:2, init_h:end_h, init_w:end_w]
...
class testset(Dataset):
    ...
    def __init__(self, name_list, coordinate_list, reg_image):
        self.name_list = name_list
        self.coordinate_list = coordinate_list
    ...
    def __getitem__(self, index):
        ...
        init_s = single_coordinate['init_s']
        end_s = single_coordinate['end_s']
        noise_patch = self.reg_image[init_s:end_s, init_h:end_h, init_w:end_w]
...
def train_preprocess_lessMemoryMulStacks(args):
    ...
    for im_name in list(os.walk(im_folder, topdown=False))[-1][-1]:
        ...
        reg_image = tiff.imread(im_dir)
        if reg_image.shape[0] > args.select_img_num:
            reg_image = reg_image[0:args.select_img_num, :, :]
        gap_t2 = get_gap_t(args, reg_image, stack_num)
        args.gap_t = gap_t2
        # print('gap_t2 -----> ',gap_t2)
        # print('reg_image shape -----> ',reg_image.shape)
        # print('reg_image max -----> ',reg_image.max())
        # print('reg_image min -----> ',reg_image.min())
        reg_image = reg_image.astype(np.float32) / args.scale_factor  # no preprocessing
        # reg_image = (reg_image-reg_image.min()).astype(np.float32)/args.scale_factor 
        reg_image_all.append(reg_image)

        whole_x = reg_image.shape[2]
        whole_y = reg_image.shape[1]
        whole_t = reg_image.shape[0]
        ...
        for x in range(0, int((whole_y - patch_y + gap_y) / gap_y)):
            for y in range(0, int((whole_x - patch_x + gap_x) / gap_x)):
                for z in range(0, int((whole_t - patch_t2 + gap_t2) / gap_t2)):
                    ...
                    # noise_patch1 = reg_image[init_s:end_s,init_h:end_h,init_w:end_w]
        ...
        ind = ind + 1;
    return name_list, reg_image_all, coordinate_list, stack_index
...
def test_preprocess_lessMemoryNoTail_chooseOne(args, reg_image):
    patch_y = args.patch_y
    patch_x = args.patch_x
    ...
    # print('reg_image max -----> ',reg_image.max())
    # print('reg_image min -----> ',reg_image.min())
    if reg_image.shape[0] > args.test_datasize:
        reg_image = reg_image[0:args.test_datasize, :, :]
    reg_image = reg_image.astype(np.float32) / args.scale_factor
    # reg_image = (reg_image-reg_image.min()).astype(np.float32)/args.scale_factor
    ...
    for x in range(0, num_h):
        for y in range(0, num_w):
            for z in range(0, num_s):
                ...
                name_list.append(patch_name)
                # print(' single_coordinate -----> ',single_coordinate)
                coordinate_list = single_coordinate

    # return name_list, reg_image, coordinate_list
    return name_list, reg_image, coordinate_list


def test_preprocess_chooseOne(args, reg_image):
    """
    ...
    Returns:
        ...
        reg_image : the original noisy stacks
    ...
    input_data_type = reg_image.dtype
    img_mean = reg_image.mean()
    # print('reg_image max -----> ',reg_image.max())
    # print('reg_image min -----> ',reg_image.min())
    if reg_image.shape[0] > args.test_datasize:
        reg_image = reg_image[0:args.test_datasize, :, :]
    if args.print_img_name:
       ...
       print('Testing image shape -----> ', reg_image.shape)
    # Minus mean before training
    reg_image = reg_image.astype(np.float32)/args.scale_factor
    reg_image = reg_image-img_mean
    # No preprocessing
    # reg_image = reg_image.astype(np.float32) / args.scale_factor
    # reg_image = (reg_image-reg_image.min()).astype(np.float32)/args.scale_factor
    ...
    return  name_list, reg_image, img_mean,  input_data_type, coordinate_list
